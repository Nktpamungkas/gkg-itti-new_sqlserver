...
<body>
    ...
    if (!empty($_POST["submit"])) {
        ...
        if ($group == "ALL") {
            ...
            GROUP by a.no_mesin, a.no_urut, a.proses
            ...
            if ($_POST["w_shift"] != 'ALL') {
                ...
                DATE_FORMAT(a.tgl_update,'%Y-%m-%d %H:%i') >= '$start_shift3' 
                AND DATE_FORMAT(a.tgl_update,'%Y-%m-%d %H:%i') <= '$end_shift3'
                GROUP by a.no_mesin, a.no_urut, a.proses
                ORDER by a.tgl_stop DESC");
                ...
                FROM tbl_schedule a
                join tbl_gerobak b on a.id = b.id_schedule
                where a.status = 'selesai' AND a.leader_check = 'TRUE' and a.g_shift = '$group' and DATE_FORMAT(a.tgl_update,'%Y-%m-%d') >= '$date_s' 
                AND DATE_FORMAT(a.tgl_update,'%Y-%m-%d') <= '$date_e'
                GROUP by a.no_mesin, a.no_urut, a.proses
                ORDER by a.tgl_stop DESC");
            }
    ...
    ?>
    <div class="row">
        <p style="font-weight: bold; color: red; font-style: italic; margin-left: 20px;">Untuk men-genarate laporan shift 3, maka ambil tanggal pada shift di mulai S/D tanggal shift berakhir dan centang checkbox shift 3
        </p>
        <div class="col-xs-12">
            <div class="box">
                                ...
                                <div class="form-group">
                                    <div class="col-sm-2">
                                        <select required name="g_shift" class="form-control input-sm" required>
                                            <option <?php if ($group == "ALL") echo "selected"; ?> value="ALL">ALL GROUP</option>
                                            <option <?php if ($group == "A") echo "selected"; ?> value="A">A</option>
                                            <option <?php if ($group == "B") echo "selected"; ?> value="B">B</option>
                                            <option <?php if ($group == "C") echo "selected"; ?> value="C">C</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
